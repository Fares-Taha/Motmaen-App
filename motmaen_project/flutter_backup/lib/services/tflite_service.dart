...
class TFLiteService {
  ...
  Future<void> loadModel() async {
    try {
      // Load TFLite model
      _interpreter = await Interpreter.fromAsset('assets/models/model_v5_fine_tuned_int8.tflite');

      // Load labels from assets
      final labels = await rootBundle.loadString('assets/models/labels.txt');
      _classNames = labels.split('\n').where((name) => name.isNotEmpty).toList();
  ...
  Future<FoodPrediction?> predictImage(File imageFile) async {
    try {
      ...
      List<List<List<List<int>>>> inputArray =
          List.generate(1, (_) => List.generate(224, (_) =>
          List.generate(224, (_) => List.generate(3, (_) => 0))));
      ...
      var output = List.filled(_classNames.length, 0).reshape([1, _classNames.length]);
